...
> `pip` と `python3` が別環境を向いていることがあるため、`pip install ...` より `python3 -m pip install ...` を推奨します。
...
* 生成された `parts/*.glb` のすべてが最終 scene で使われるとは限りません
* たとえば TB3 では `caster_back_link.glb` は生成されますが、scene 側で参照しない構成でも問題ありません
...
* ここで生成されるのは canonical な生成物名です
* TB3 テンプレートへ配置するときは、後段で `endpoint_shm_poll_with_pdu.json` という project-side の名前に合わせて staging します
...
現在は `godot_scene.yaml` に従って、以下のような Godot scene 側の方針も反映します。

* `HakoSync` 用の薄い GDScript 生成
* 必要に応じた `HakoniwaSimNode` / `HakoniwaCodecNode` の挿入
* 最小限の default material の付与
* `assets/` / `config/` 構成に合わせた resource path
...
> **自分のロボットでは：**
> 7-1 で生成した viewer model JSON を使います。scene 側の出力方針は `bodies/{name}/config/godot_scene.yaml` で調整します。

補足:

* `camera_rig.gd` と `Camera3D` は generator の責務に含めていません
* カメラ操作を付けたい場合は、後段で Godot editor 上から追加します
...
mkdir -p "$GODOT_PROJECT_DIR/assets/parts"
mkdir -p "$GODOT_PROJECT_DIR/config/comm"
mkdir -p "$GODOT_PROJECT_DIR/config/cache"

cp -f bodies/turtlebot3/generated/parts/*.glb \
  "$GODOT_PROJECT_DIR/assets/parts/"

cp -f bodies/turtlebot3/godot_tb3_reference/TurtleBot3.generated.tscn \
  "$GODOT_PROJECT_DIR/assets/"
...
cp -f bodies/turtlebot3/generated/endpoint_shm_with_pdu.json \
  "$GODOT_PROJECT_DIR/config/endpoint_shm_poll_with_pdu.json"

cp -f bodies/turtlebot3/generated/godot/robot_sync.profile.json \
  "$GODOT_PROJECT_DIR/config/"

cp -f bodies/turtlebot3/generated/pdutypes.json \
  "$GODOT_PROJECT_DIR/config/comm/tb3-pdutypes.json"

cp -f bodies/turtlebot3/generated/pdu_def.json \
  "$GODOT_PROJECT_DIR/config/comm/tb3-pdudef-compact.json"
```
...
* `pdutypes.json` / `pdu_def.json` / `endpoint_shm_with_pdu.json` は canonical な生成物名です
* TB3 テンプレートでは project-side の staging 名として以下を使います
  * `config/comm/tb3-pdutypes.json`
...
TB3 テンプレート相当のカメラ操作を付けたい場合は、Godot editor 上で `camera_rig.gd` を使って手動追加します。

最低限の手順:

1. `CameraRig` という `Node3D` を scene root 直下に追加する
2. `Camera3D` を `CameraRig` の子として追加する
3. `camera_rig.gd` を `CameraRig` にアタッチする
4. `camera_path` を `Camera3D` に設定する
5. `target_path` を `RosToGodot/Visuals/base_link` に設定する

> `camera_rig.gd` 自体は sample / tutorial 側の資産として扱う想定です。
...
TB3 テンプレートへ staging した後の代表名:

| ファイル                                                   | 用途                             |
| ------------------------------------------------------ | ------------------------------ |
| `assets/TurtleBot3.generated.tscn`                     | Godot シーン                      |
| `assets/parts/*.glb`                                   | Godot 用パーツ GLB                 |
| `assets/tb3_reference_sync.gd`                         | HakoSync 用 GDScript            |
| `config/robot_sync.profile.json`                       | Godot robot sync profile       |
| `config/endpoint_shm_poll_with_pdu.json`               | Godot 用 endpoint 設定            |
| `config/comm/tb3-pdutypes.json`                        | TB3 用 staged PDU 型定義         |
| `config/comm/tb3-pdudef-compact.json`                  | TB3 用 staged compact PDU 定義 |
...
* 生成された `assets/TurtleBot3.generated.tscn` が Godot に読み込まれていること
* シーン内に `assets/parts/*.glb` が割り当てられ、ロボットの形状が表示されること
* `config/endpoint_shm_poll_with_pdu.json` が配置されていること
* `config/robot_sync.profile.json` が配置されていること
* シーン内に `assets/tb3_reference_sync.gd` が設定されていること

目視での静的チェック:

* `.tscn` の `ext_resource` が `res://assets/parts/...` と `res://assets/tb3_reference_sync.gd` を参照していること
* `HakoSync.profile_path` が `res://config/robot_sync.profile.json` を指していること
* `HakoniwaSimNode.shm_endpoint_config_path` が `res://config/endpoint_shm_poll_with_pdu.json` を指していること
* `config/comm/tb3-pdudef-compact.json` の `path` が `tb3-pdutypes.json` になっていること
* `config/endpoint_shm_poll_with_pdu.json` の `comm` が `comm/shm_poll_comm.json` を指していること
* `config/endpoint_shm_poll_with_pdu.json` の `pdu_def_path` が `comm/tb3-pdudef-compact.json` を指していること
...
* `assets/parts/*.glb` が Godot プロジェクトの `assets/parts/` ディレクトリにコピーされているか
* Godot シーンに `WorldEnvironment` と `DirectionalLight3D` が含まれているか
* GLB に material が含まれていない場合でも、scene generator が default material を付与しているか
* `.tscn` 内の resource path が `res://assets/...` 構成と一致しているか
...
* `endpoint_shm_poll_with_pdu.json` の参照先が staging 後の `config/comm/*.json` と一致しているか
...
* `config/comm/tb3-pdudef-compact.json` が `tb3-pdutypes.json` を参照しているか
